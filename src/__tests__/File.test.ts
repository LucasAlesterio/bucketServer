...
const pathTest = ["test", "subTest"];
const namesTest = ["TestImage1", "TestImage2"];
describe("Files", () => {
    it("Should be able to save the image", async () => {
    ...
    it("Shold be able to get images", async () => {
        const compleatPath = pathTest.reduce(
            (initial: string, current: string) => `${initial}/${current}`
        );
        namesTest.forEach(async (name) => {
            const response = await request(app).get(
                `/${compleatPath}/${name}.png`
            );
